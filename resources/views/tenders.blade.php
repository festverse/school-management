<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Tenders & Public Procurement - Lumina University</title>
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=inter:400,500,600,700,800|outfit:400,600,800&display=swap" rel="stylesheet" />
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = {
            darkMode: 'class',
            theme: {
                extend: {
                    fontFamily: { sans: ['Inter', 'sans-serif'], display: ['Outfit', 'sans-serif'] },
                    colors: { primary: { 50: '#eff6ff', 100: '#dbeafe', 500: '#3b82f6', 600: '#2563eb', 900: '#1e3a8a' } }
                }
            }
        }
    </script>
    <style>
        .glass-nav { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(255, 255, 255, 0.1); }
        .hero-gradient { background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%); }
        .text-gradient { background: linear-gradient(to right, #60a5fa, #a78bfa); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    </style>
</head>
<body class="antialiased bg-gray-900 text-gray-100 font-sans selection:bg-primary-500 selection:text-white">
    <!-- Navbar -->
    <nav class="fixed w-full z-50 glass-nav transition-all duration-300">
        <div class="w-full px-4 sm:px-8 lg:px-12">
            <div class="flex justify-between h-20 items-center">
                <a href="{{ url('/') }}" class="flex-shrink-0 flex items-center gap-3 group">
                    <div class="text-3xl group-hover:scale-110 transition-transform duration-300">🎓</div>
                    <span class="font-display font-bold text-2xl tracking-tight text-white group-hover:text-primary-400 transition-colors">Lumina University</span>
                </a>
                <div class="hidden md:flex items-center space-x-8">
                    <a href="{{ route('academics') }}" class="text-sm font-medium text-gray-300 hover:text-white transition-colors">Academics</a>
                    <a href="{{ route('admissions') }}" class="text-sm font-medium text-gray-300 hover:text-white transition-colors">Admissions</a>
                    <a href="{{ url('/#student-directory') }}" class="text-sm font-medium text-gray-300 hover:text-white transition-colors">Student Directory</a>
                    <div class="h-6 w-px bg-gray-700"></div>
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-medium text-white bg-primary-600 rounded-full hover:bg-primary-500 transition-all shadow-lg shadow-primary-500/25 hover:shadow-primary-500/40 hover:-translate-y-0.5">Enter Portal</a>
                        @else
                            <a href="{{ route('login') }}" class="text-sm font-medium text-gray-300 hover:text-white transition-colors">Log in</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-medium text-white bg-white/10 border border-white/20 rounded-full hover:bg-white/20 transition-all backdrop-blur-sm">Apply Now</a>
                            @endif
                        @endauth
                    @endif
                </div>
            </div>
        </div>
    </nav>

    <!-- Hero Section -->
    <div class="relative pt-32 pb-20 hero-gradient flex items-center overflow-hidden border-b border-gray-800">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary-600/10 rounded-full blur-3xl opacity-40 pointer-events-none"></div>
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 class="text-4xl md:text-6xl font-display font-extrabold tracking-tight text-white mb-6">
                Tenders & <span class="text-gradient">Public Procurement</span>
            </h1>
            <p class="max-w-2xl text-lg text-gray-400 mx-auto font-light">
                Official institutional notice board for open tenders, Request for Proposals (RFPs), laboratory equipment procurement, and infrastructure development contracts.
            </p>
        </div>
    </div>

    <!-- Tenders Table Section -->
    <div class="py-24 bg-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            <div class="bg-gray-800/40 border border-gray-700/50 rounded-3xl overflow-hidden shadow-2xl p-8">
                <h2 class="text-2xl font-display font-bold text-white mb-6">Active Tenders (2026-2027)</h2>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-700 text-gray-400 text-sm">
                                <th class="py-4 px-6 font-semibold">Tender No. / Ref ID</th>
                                <th class="py-4 px-6 font-semibold">Name of Work / Procurement</th>
                                <th class="py-4 px-6 font-semibold">Estimated Cost</th>
                                <th class="py-4 px-6 font-semibold">Closing Date</th>
                                <th class="py-4 px-6 font-semibold">Action</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-700/50 text-sm text-gray-300">
                            <tr class="hover:bg-gray-800/50 transition-colors">
                                <td class="py-5 px-6 font-medium text-white">LU/TEN/2026/089</td>
                                <td class="py-5 px-6">Supply, Installation, and Commissioning of 500-Node High Performance GPU Supercomputing Cluster for AI CoE.</td>
                                <td class="py-5 px-6 font-semibold text-emerald-400">₹14,50,00,000</td>
                                <td class="py-5 px-6 text-gray-400">15th August 2026, 17:00 IST</td>
                                <td class="py-5 px-6"><button onclick="alert('Tender document encryption key verified. Downloading official RFP zip file...')" class="px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-primary-500/20">Download RFP</button></td>
                            </tr>
                            <tr class="hover:bg-gray-800/50 transition-colors">
                                <td class="py-5 px-6 font-medium text-white">LU/TEN/2026/074</td>
                                <td class="py-5 px-6">Construction of Advanced Mechatronics & Robotics Laboratory Complex (Package-II Building Works).</td>
                                <td class="py-5 px-6 font-semibold text-emerald-400">₹28,80,00,000</td>
                                <td class="py-5 px-6 text-gray-400">30th July 2026, 15:00 IST</td>
                                <td class="py-5 px-6"><button onclick="alert('Tender document encryption key verified. Downloading official RFP zip file...')" class="px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-primary-500/20">Download RFP</button></td>
                            </tr>
                            <tr class="hover:bg-gray-800/50 transition-colors">
                                <td class="py-5 px-6 font-medium text-white">LU/TEN/2026/062</td>
                                <td class="py-5 px-6">Annual Maintenance Contract (AMC) for Campus-Wide Optical Fiber Gigabit Backbone Network & Cisco Core Switches.</td>
                                <td class="py-5 px-6 font-semibold text-emerald-400">₹3,20,00,000</td>
                                <td class="py-5 px-6 text-gray-400">22nd July 2026, 14:00 IST</td>
                                <td class="py-5 px-6"><button onclick="alert('Tender document encryption key verified. Downloading official RFP zip file...')" class="px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-primary-500/20">Download RFP</button></td>
                            </tr>
                            <tr class="hover:bg-gray-800/50 transition-colors">
                                <td class="py-5 px-6 font-medium text-white">LU/TEN/2026/051</td>
                                <td class="py-5 px-6">Procurement of Cryogenic Electron Microscope (Cryo-EM) for Nanotechnology Research Center.</td>
                                <td class="py-5 px-6 font-semibold text-emerald-400">₹18,00,00,000</td>
                                <td class="py-5 px-6 text-gray-400">10th July 2026, 17:00 IST</td>
                                <td class="py-5 px-6"><button onclick="alert('Tender document encryption key verified. Downloading official RFP zip file...')" class="px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-primary-500/20">Download RFP</button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>

            <!-- Pre-Bid Guidelines -->
            <div class="bg-gray-800/20 border border-gray-800 rounded-3xl p-10 max-w-4xl mx-auto text-center space-y-6">
                <h3 class="text-2xl font-bold text-white">Important Instructions for Bidders</h3>
                <p class="text-gray-400 text-sm leading-relaxed">All registered vendors and engineering contractors must submit their technical bids and commercial quotations via the national e-Procurement portal. Earnest Money Deposit (EMD) must be submitted via bank guarantee in favor of the Registrar, Lumina University.</p>
                <div class="pt-2">
                    <a href="{{ route('contact') }}" class="inline-flex items-center px-6 py-3 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white rounded-xl font-medium transition-all">Contact Tender Office</a>
                </div>
            </div>
        </div>
    </div>

    <!-- Master Footer -->
    <x-footer />
</body>
</html>
